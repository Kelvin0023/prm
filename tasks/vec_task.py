...
class VecTask(Env):
    ...
    def __init__(
        ...
        Args:
            ...
            sim_device:
                the device to simulate physics on. eg. 'cuda:0' or 'cpu'
            graphics_device_id:
                the device ID to render with.
            headless:
                Set to False to disable viewer rendering.
            virtual_screen_capture:
                Set to True to allow the users get captured screen in RGB array via `env.render(mode='rgb_array')`.
            force_render:
                Set to True to always force rendering in the steps (if the `control_freq_inv` is greater
                than 1 we suggest stting this arg to True)
        """
        ...
        print("Creating sim...may take a while.")
        self.create_sim()
    ...
    def get_state(self):
        ...
        return self.states_buf.clone().to(self.rl_device)

    def get_obs(self):
        """Returns the obs buffer of the environment"""
    ...
    def step(self, actions: torch.Tensor) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, torch.Tensor, Dict[str, Any]]:
        ...
        # fill time out buffer: set to 1 if we reached the max episode length AND the reset buffer is 1. Timeout == 1
        # makes sense only if the reset buffer is 1.
        ...
        self.obs_dict["obs"] = self.get_obs()
    ...
    def reset_idx(self, env_idx):
        """Reset environment with indces in env_ids.
